  ...
  try {
    ...
    if ((await isEmailExist) === null) {
      ...
      return {
        success: true,
        data: {
          email: await userResp.email,
          user_id: await userResp.user_id,
          address_id: addressResp.address_id,
        },
...
exports.viewInvoiceDetailsServices = async (orderId) => {
  try {
    let query = `select full_name,email,order_details.product_id,products.user_id,ordered_qty,price,unit_price,product_name,descriptions,contact,apartment,city,street,pin_code,state,address_type from users inner join orders on users.user_id =orders.user_id inner join order_details on orders.order_id=order_details.order_id inner join products on order_details.product_id=products.product_id inner join addresses on order_details.address_id=addresses.address_id where orders.order_id=${orderId};`;
