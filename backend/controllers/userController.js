...
exports.viewInvoiceDetails = async (req, res, next) => {
  let orderId = req.params.id;
  try {
    let invoice = await viewInvoiceDetailsServices(orderId);
    if (invoice) {
      return res.status(200).json({
        status: true,
        data: invoice,
        message: "Successfully invoice details Retrieved",
      });
    }  else {
